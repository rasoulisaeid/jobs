...
(function () {
  ...
  const NODE_URL  = `${DB_URL}/jobs/${USER_KEY}/data.json`;
  ...
  const status = { online: navigator.onLine, lastPush: null, error: null };

  function emit() { window.dispatchEvent(new CustomEvent("jobs-sync-status", { detail: { ...status } })); }
  ...
  async function pullOnce() {
    ...
    try {
      ...
      console.warn("Jobs: pull failed —", e.message);
    }
    emit();
  }
  ...
  function schedulePush() {
    ...
    pushTimer = setTimeout(async () => {
      ...
      try {
        ...
        status.error = e.message;
        console.error("Jobs: save failed —", e.message);
      }
      emit();
    }, 700);
  }
  ...
  function subscribeSSE() {
    ...
    } catch (e) { console.warn("Jobs: live sync unavailable —", e.message); }
  }
